...
function setupIpcHandlers() {
  import_electron.ipcMain.handle("get-system-drives", async () => {
    try {
      const isWindows = process.platform === "win32";
      if (isWindows) {
        const { stdout } = await execAsync("wmic logicaldisk get name,drivetype,volumename", { timeout: 5e3 });
  ...
  import_electron.ipcMain.handle("abort-search", async () => {
  ...
  import_electron.ipcMain.handle("open-folder", async (_event, folderPath) => {
    try {
      ...
      (0, import_child_process.exec)(command, execOptions, (error) => {
        if (error) {
